...
for k, fname in enumerate(ids):
    print(k, end='\r') 
    myfile = os.path.join(args.dest_path,fname +'.xml')
    myfile = Path(myfile)
    if not myfile.exists(): #if file is not existing 
        txtfile = os.path.join(args.sourcepath, fname + '.txt') #Read annotation of each image from txt file
        f = open(txtfile,"r")
        ...
        for x in f:     #Iterate for each object in a image. 
            ...
            value_xmin = float(x[1])*img.shape[1]
            ...
            value_ymax = float(x[4])*img.shape[0]
            child_ymax.text = str(int(value_ymax)) #ymax
